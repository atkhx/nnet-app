...
func CreateGrayscaleImagesFromDataMatrixesWithAverageValues(m *data.Volume) ([][]byte, error) {
	mh := int(math.Sqrt(float64(m.W)))
	mw := mh
	res := [][]byte{}

	for imageIndex := 0; imageIndex < m.D*m.H; imageIndex++ {
		imgFloats := m.GetRowByAbsoluteIndex(imageIndex).Data
		//imgFloats := data.MatrixRowFloats(m.W, imageIndex, m.Data)

		img := image.NewGray(image.Rect(0, 0, mw, mh))
		min, max := data.GetMinMaxValues(imgFloats)
		max -= min
...
func CreateRGBImagesFromDataMatrixesWithAverageValues(m *data.Volume) ([][]byte, error) {
	mh := int(math.Sqrt(float64(m.W)))
	mw := mh
	res := [][]byte{}
	imageSQ := mw * mh

	for imageIndex := 0; imageIndex < m.D; imageIndex++ {
		imgFloats := m.Data[imageIndex*m.W*3 : (imageIndex+1)*m.W*3]
